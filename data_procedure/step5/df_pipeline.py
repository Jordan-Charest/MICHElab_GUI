...
def return_dfc_filename(mouse_num, id, signal_str):
    return f"../data/M{mouse_num}_{id}_{signal_str}_dfc.h5"

def create_dataframe(mice_num, id):
...
def mouse_directory(mouse_num):
    return f"../data/M{mouse_num}"

dataframe_directory = "../data/dataframes"

# General params
mice_num = []
data_ids = ["mvmt", "no_mvmt", "trim_mvmt"]
file_id = "v1" # File identifier. Files are formatted as "M{mouse_num}_{file_id}_{data_id}.h5", for example "M308_v1_mvmt.h5".

# Null model params
null_models = ["null_normal", "null_shuffle"]
...
for mouse_num in mice_num: # For each mouse
    ...
    for null_model in null_models: # For each null_model that has to be computed
        ...
        if null_model == "null_normal": # Generate a null model with a normal distribution
            ...
            random_noise = np.random_normal(normal_params[0], normal_params[1], data.shape) # Generate random noise of the same shape as mouse data
            nan_mask = np.where(np.isnan(data[0,:,:]), np.nan, 1) # Compute mask of nan values in ouse data
    ...
    del data, random_noise, nan_mask, masked_random_noise, random_data
...
# 4 - Compute segments
# Using the computed dFC files, compute the segments. mats and indices. There will be one of each for each mouse and data_id combination
segment_mats = [[] for i in data_ids]
segment_indices = [[] for i in data_ids]

for i, data_id in enumerate(data_ids):
    for mouse_num in mice_num:
        
        cts, _ = get_data_from_dataset(dfc_filename(mouse_num, f"{file_id}_{data_id}", segmenting_str), "cts")

        mats, indices = split_into_bins(cts, n_segments)

        segment_mats[i].append(mats)
        segment_indices[i].append(indices)
...
for data_id_index, data_id in enumerate(data_ids): # For each model we want to consider (INCLUDING null models)
    ...
    base_df_filename = os.path.join(dataframe_directory, f"{file_id}_{data_id}_df.csv")
    ...
    for name in ["nvc", "modularity", "funcsim", "funcrep"]:
        copy_dataframe(base_df_filename, name+base_df_filename)

    indexes = [f"M{mouse_num}_{file_id}_{data_id}" for mouse_num in mice_num]

    ### NEUROVASCULAR COUPLING (nvc)

    df = return_dataframe("nvc"+base_df_filename) # Load appropriate dataframe
    ...
    for i, mouse_num in enumerate(mice_num):
        ...
        GCaMP_signal, _ = get_data_from_dataset(return_mouse_filename(mouse_num, f"{file_id}_{data_id}"), f"data/3d/GCaMP")
        HbT_signal, _ = get_data_from_dataset(return_mouse_filename(mouse_num, f"{file_id}_{data_id}"), "data/3d/dHbT")
        ...
        for j, indices in enumerate(segment_indices[data_id_index][i]): # The j-th segment of mouse i
    ...
    df.to_csv("nvc"+base_df_filename) # save modified dataframe
    ...
    for i, mouse_num in enumerate(mice_num):
        ...
        GCaMP_dfc, _ = get_data_from_dataset(return_dfc_filename(mouse_num, f"{file_id}_{data_id}", "GCaMP"), "dfc")
        dHbT_dfc, _ = get_data_from_dataset(return_dfc_filename(mouse_num, f"{file_id}_{data_id}", "dHbT"), "dfc")
    ...
    funcrep_filename = return_dataframe("funcrep"+base_df_filename)
    modularity_df = return_dataframe("modularity"+base_df_filename)
    funcsim_df = return_dataframe("funcsim"+base_df_filename)
    ...
    funcsim_df.to_csv("funcsim"+base_df_filename)
    ...
    funcrep_df.to_csv("funcrep"+base_df_filename)
    ...
    modularity_df.to_csv("modularity"+base_df_filename)
