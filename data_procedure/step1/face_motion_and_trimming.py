...
for mouse_num in mice_num:
    ...
    video_input, video_output, face_motion_output, HbT_input, datafile_input = return_filepaths(mouse_num)
    ...
    HbT_signal = read_data(HbT_input)
    duration = HbT_signal.shape[0] # At 12 fps, should be 5760 (8 minutes). 1440 for 3 fps.
    print(f"HbT data with {duration} frames loaded. Trimming the video and face motion to match.")

    HbT_signal = np.nanmean(HbT_signal, axis=(1, 2))

    face_motion_signal = np.load(face_motion_output)

    # Find the indices that correctly spread the monitoring frames across the HbT timeseries
    indices_vec = np.arange(frame_range[0], frame_range[1], 1)
    indices = evenly_spread_vector_elements(indices_vec, duration)

    # Cross correlate face_motion with HbT
    correlations = compute_correlation_with_lag(HbT_signal, face_motion_signal, indices, lag_range, abs_r=abs_r)
