...
from toolbox_jocha.connectivity import bin_3d_matrix, bin_2d_matrix
...
from toolbox_jocha.regions import select_region_pixels, select_dfc_indices
import shutil
# import bct
...
def return_mouse_filename(mouse_num, id):
    return f"/home/jocha337/links/projects/def-mides323/jocha337/TO_CC/M{mouse_num}_{id}.h5"

def return_dfc_filename(mouse_num, id, signal_str):
    return f"/home/jocha337/links/projects/def-mides323/jocha337/TO_CC/M{mouse_num}_{id}_{signal_str}_dfc.h5"
...
def mouse_directory(mouse_num):
    return f"/home/jocha337/links/projects/def-mides323/jocha337/TO_CC/"

dataframe_directory = "/home/jocha337/links/projects/def-mides323/jocha337/TO_CC/dataframes/"

# General params
mice_num = list(sys.argv[1].split(","))
data_ids = list(sys.argv[2].split(",")) # Typically mvmt, no_mvmt, trim_mvmt
# data_ids = ["mvmt"]
file_id = "v2" # File identifier. Files are formatted as "M{mouse_num}_{file_id}_{data_id}.h5", for example "M308_v1_mvmt.h5".

# Null model params
null_models = list(sys.argv[3].split(",")) # Typically null_normal, null_shuffle
if null_models[0] == "None":
    null_models = []
# null_models = []
apply_shuffle_to = "mvmt"
...
# Other
n_mice = len(mice_num)
max_region_label = 20 # Highest region label expected. Useful for initializing some arrays.
...
for data_id_index, data_id in enumerate(data_ids): # For each model we want to consider (INCLUDING null models)
    ...
    for registration_str in ["whole", "registered"]:
        ...
        if registration_str == "registered":
        ...
        for i, mouse_num in enumerate(mice_num):
            ...
            atlas_raw, atlas_attr = get_data_from_dataset(return_mouse_filename(mouse_num, f"{file_id}_{data_id}"), f"registration/atlas")
            regions_labels = atlas_attr["region_labels"].tolist()
            ...
            for j, indices in enumerate(segment_indices[registration_str][data_id_index][i]): # The j-th segment of mouse i
                ...
                if registration_str == "registered": # Process for each region individually as well

                    print(f"Raw atlas shape: {atlas_raw.shape}")
                    print(f"Expected shape (GCaMP) ({GCaMP_signal.shape[1]},{GCaMP_signal.shape[2]}):")

                    if atlas_raw.shape != (GCaMP_signal.shape[1], GCaMP_signal.shape[2]):
                        raise ValueError("Mismatch between loaded atlas shape and data shape. Please verify that the provided atlas is correct. Skipping.")
                    
                    # TODO: uncomment to perform calculations

                    # for region_i in range(len(regions_labels)):

                    #     region_label = int(regions_labels[i][0])
                    #     region_name = regions_labels[i][1]

                    #     regions_names_array[region_label] = f"{region_label}_{region_name}"

                    #     region_GCaMP_signal = select_region_pixels(atlas_raw, region_label, sliced_GCaMP_signal)
                    #     region_HbT_signal = select_region_pixels(atlas_raw, region_label, sliced_HbT_signal)

                    #     lag_mat, correlation_mat = r_coeff_2mats(region_GCaMP_signal, region_HbT_signal, max_shift=max_shift, lag=compute_lag, convert_to_s=convert_to_s, fps=fps, squared=squared_r, lag_sign=lag_sign)

                    #     regions_neurovascular_coupling[i,region_i,j] = np.nanmean(correlation_mat) # Mouse i, region region_i, segment j
                    #     regions_lag[i,region_i,j] = np.nanmean(lag_mat)

                    
                    # mask = regions_names_array!='' # Indices where there are regions (for slicing)
                    # regions_names_array = regions_names_array[mask]

                    # regions_neurovascular_coupling = regions_neurovascular_coupling[:,mask,:]
                    # regions_lag = regions_lag[:,mask,:]


                    # TODO: SAVE THIS SHIT (and add prints for validation?)
        ...
        if registration_str == "registered":
            regions_names_array = np.empty(max_region_label+1, dtype=str) # To store region labels
            regions_funcsim = np.zeros((n_mice, max_region_label+1, n_segments))*np.nan
            regions_GCaMP_funcrep = np.zeros((n_mice, max_region_label+1, n_segments))*np.nan
            regions_dHbT_funcrep = np.zeros((n_mice, max_region_label+1, n_segments))*np.nan
            regions_GCaMP_mod = np.zeros((n_mice, max_region_label+1, n_segments))*np.nan
            regions_dHbT_mod = np.zeros((n_mice, max_region_label+1, n_segments))*np.nan
        ...
        for i, mouse_num in enumerate(mice_num):
            ...
            atlas_raw, atlas_attr = get_data_from_dataset(return_mouse_filename(mouse_num, f"{file_id}_{data_id}"), f"registration/atlas")
            print(f"Raw atlas shape: {atlas_raw.shape}")
            atlas_binned = bin_2d_matrix(atlas_raw, (y_binning, x_binning), method="skip")
            print(f"Binned atlas_shape: {atlas_binned.shape}")
            GCaMP_signal, _ = get_data_from_dataset(return_mouse_filename(mouse_num, f"{file_id}_{data_id}"), f"{registration_path}/GCaMP")
            binned_signal = bin_2d_matrix(GCaMP_signal[0], (y_binning, x_binning))
            print(f"Expected shape (binned GCaMP): {binned_signal.shape}")
            remove_indices = np.where(np.isnan(binned_signal[0])) # Where there are nans in the first frame of signals
            atlas_binned[remove_indices] = np.nan
            regions_labels = atlas_attr["region_labels"].tolist()

            del GCaMP_signal, remove_indices, atlas_raw, binned_signal

            # TODO: Shape check

            if registration_str == "registered":

                flattened = atlas_binned.flatten()
                y = np.isnan(flattened)
                non_nan_flattened = flattened[~y]

                print(f"Flattened atlas with nan removed shape: {non_nan_flattened.shape}")
                print(f"Expected (GCaMP dfc shape): {GCaMP_dfc.shape}")

                if non_nan_flattened.shape[0] != GCaMP_dfc.shape[1]: # The number of pixels is not the same

                    raise ValueError(f"Mismatch between atlas ({non_nan_flattened.shape[0]}) and dfc ({GCaMP_dfc.shape[1]}) shape.")
                
                # TODO: Compute and save this shit
            ...
            for j, indices in enumerate(segment_indices[registration_str][data_id_index][i]): # The j-th segment of mouse i
                ...
                if registration_str == "registered": # Process for each region individually as well
                    ...
                    for region_i in range(len(regions_labels)):
                        ...
                        region_GCaMP_dfc = select_dfc_indices(atlas_binned, region_label, sliced_GCaMP_dfc)
                        region_dHbT_dfc = select_dfc_indices(atlas_binned, region_label, sliced_dHbT_dfc)
